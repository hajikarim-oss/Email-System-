...
async function main() {
  console.log("Updating dispatched flags on the 48 Smartlead leads...");
  ...
  console.log(`✓ Audit: Total leads in Q3 = ${finalCount}, Dispatched = ${finalSent}, Pending = ${finalCount - finalSent}`);

  // Fetch top 5 leads ordered by lastContactedAt desc
  const dispatchedSample = await prisma.lead.findMany({
    where: { campaignId: "cmp_1790233732719_dvlj", totalOutbound: { gt: 0 } },
    take: 5,
    orderBy: { lastContactedAt: "asc" },
    select: { email: true, firstName: true, domain: true, customData: true, totalOutbound: true }
  });
  console.log("First dispatched leads in sequence:", dispatchedSample);
