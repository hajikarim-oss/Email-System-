...
export function CreditsMeter() {
    ...
    const fraction = spendWindow
        ? Math.max(0, 1 - (spendWindow.spent ?? 0) / Math.max(1, spendWindow.limit ?? 1))
        : (c.monthly_allowance ?? 0) > 0
          ? Math.min(1, (c.monthly_balance ?? 0) / (c.monthly_allowance || 1))
          : (total ?? 0) > 0
            ? 1
            : 0;

    const label = spendWindow
        ? `${(spendWindow.spent ?? 0).toLocaleString()} of ${(spendWindow.limit ?? 0).toLocaleString()} credits used ${spendWindow.word}`
        : `${(c.monthly_balance ?? 0).toLocaleString()} of ${(c.monthly_allowance ?? 0).toLocaleString()} plan credits left`;
    const extraLabel =
        (c.purchased_balance ?? 0) > 0 ? `, plus ${(c.purchased_balance ?? 0).toLocaleString()} extra` : "";
            ...
            <div>
                <div className="flex items-baseline justify-between text-[11.5px]">
                    ...
                    <span className="tabular-nums text-slate-900 font-medium">
                        {(credits.monthly_balance ?? 0).toLocaleString()}
                        <span className="text-slate-400 font-normal">
                            {" "}
                            / {(credits.monthly_allowance ?? 0).toLocaleString()}
                        </span>
                    </span>
                </div>
                <DitherMeter
            ...
            <div className="flex items-baseline justify-between text-[11.5px]">
                ...
                <span
                    className={cn(
                        ...
                        (credits.purchased_balance ?? 0) > 0 ? "text-slate-900" : "text-slate-400",
                    )}
                >
                    {(credits.purchased_balance ?? 0).toLocaleString()}
        ...
        <div>
            <div className="flex items-baseline justify-between text-[11.5px]">
                ...
                <span className="tabular-nums text-slate-900 font-medium">
                    ...
                    {limit != null && (
                        <span className="text-slate-400 font-normal"> of {(limit ?? 0).toLocaleString()}</span>
                    )}
                </span>
            </div>
            {fraction != null && (
