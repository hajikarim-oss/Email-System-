...
function SwitchNode({ id, data, selected }: NodeProps) {
    ...
    return (
        <div
            className={`w-[248px] rounded-xl border bg-white shadow-sm transition-shadow duration-200 hover:shadow-md ${d.orphan ? "border-dashed border-amber-300" : "border-purple-200"
                        ...
                        return (
                            <div key={caseKey(c)} className="relative flex h-6 items-center pl-2.5 pr-4">
                                ...
                                <Handle
                                    ...
                                    id={caseHandleId(c)}
                                    position={Position.Right}
                                    className={`!absolute !-right-1.5 !top-1/2 !h-3 !w-3 !-translate-y-1/2 pointer-coarse:!h-5 pointer-coarse:!w-5 !border-2 !border-white ${on ? "!bg-purple-500" : "!bg-slate-300"
                                        }`}
                                />
                            </div>
                        );
...
export default function CampaignFlow({ campaignId }: { campaignId: string }) {
    ...
    return (
        <div
            className={`campaign-flow relative h-[70dvh] w-full overflow-hidden rounded-md border border-slate-200 bg-slate-50/40 sm:h-[78vh] ${dragging ? "rf-dragging" : ""
                }`}
            onPointerMove={(e) => {
...
function StopOnReplyToggle({ on, onToggle }: { on: boolean; onToggle: (next: boolean) => void }) {
    return (
        ...
        >
            ...
            >
                <span
                    className={`inline-block size-4 transform rounded-full bg-white shadow-sm transition-transform ${on ? "translate-x-4" : "translate-x-0.5"
                        }`}
                />
            </button>
        </div>
        ...
        <div className="absolute right-3 top-3 z-20 w-[300px] max-w-[calc(100vw-1.5rem)] max-h-[calc(100%-1.5rem)] overflow-y-auto rounded-md border border-slate-200 bg-white p-3 shadow-[0_12px_32px_-8px_rgba(15,23,42,0.18)]">
            ...
            <div className="space-y-2 text-[12px] text-slate-600">
                ...
                {isInstantCapable && (
                    <div className="space-y-1">
                        <div
                            className={`flex items-center gap-2 rounded-md px-2 py-1.5 ring-1 transition-colors ${instant ? "bg-violet-50 ring-violet-200" : "bg-slate-50 ring-slate-200"
                                }`}
                        >
                            <ZapIcon
                                className={`w-3.5 h-3.5 shrink-0 transition-colors ${instant ? "text-violet-600" : "text-slate-400"
                                    }`}
                            />
                            <span
                                className={`min-w-0 flex-1 text-[11px] font-medium leading-tight transition-colors ${instant ? "text-violet-700" : "text-slate-500"
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`relative inline-flex h-[18px] w-8 shrink-0 items-center rounded-full transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-300 ${instant ? "bg-violet-600" : "bg-slate-300"
                                    }`}
                            >
                                <span
                                    className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow-sm transition-transform duration-200 ${instant ? "translate-x-[15px]" : "translate-x-[2px]"
                                        }`}
                                />
                            </button>
                        </div>
                    ...
                    {(
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-start gap-1.5 rounded-md border px-2 py-1.5 text-left transition-colors ${active
...
function AIContextToggle({
    ...
    return (
        <div
            className={`flex items-center gap-2 rounded-md px-2 py-1.5 ring-1 transition-colors ${on ? "bg-purple-50 ring-purple-200" : "bg-slate-50 ring-slate-200"
                }`}
        >
            ...
            <button
                ...
                aria-label={label}
                onClick={onToggle}
                className={`relative inline-flex h-[18px] w-8 shrink-0 items-center rounded-full transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-300 ${on ? "bg-purple-600" : "bg-slate-300"
                    }`}
            >
                <span
                    className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow-sm transition-transform duration-200 ${on ? "translate-x-[15px]" : "translate-x-[2px]"
                        }`}
                />
            </button>
        </div>
                ...
                <div className="absolute left-0 top-full z-30 mt-1 max-h-56 w-24 overflow-y-auto rounded-md border border-slate-200 bg-white py-1 shadow-[0_12px_32px_-8px_rgba(15,23,42,0.18)]">
                    {DEAL_CURRENCIES.map((c) => (
                        <button
                            ...
                            className={`flex w-full items-center px-2.5 py-1.5 text-left text-[12px] transition-colors hover:bg-slate-100 ${c === value ? "font-medium text-slate-900" : "text-slate-700"
                                }`}
                        >
                            {c}
                        </button>
